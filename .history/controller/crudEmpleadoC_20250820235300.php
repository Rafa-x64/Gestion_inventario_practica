...
class crudEmpleadoC extends mainModel
{
    ...
    public static function obtener_formulario($form)
    {
        ...
        $id = self::sanitizarIDEmpleado($form["empleado_id"]);
        //sanitizar nombre
        $nombre = self::sanitizarNombre($form["empleado_nombre"]);
        //sanitizar cedula
        $cedula = self::sanitizarCedula($form["empleado_cedula"]);
        ...
        //sanitizar motivo bonificacion
        $motivoBonificacion = self::sanitizarMotivoBonificaciones($form["empleado_motivo_bonificacion"]);
        echo $motivoBonificacion;
        //sanitizar tipo de la deduccion
        $tipoDeduccion = self::sanitizarTipoDeduccion($form["empleado_tipo_deduccion"]);
        //monto de la deduccion
        $montoDeduccion = self::sanitizarMontoDeduccion($form["empleado_deduccion_monto"]);
        //sanitizar tipo de pago
        $tipoPago = self::sanitizarTipoPago($form["empleado_tipo_pago"]);
        //sanitizar banco
        $banco = self::sanitizarBanco($form["empleado_banco"]);
        //sanitizar numero de cuenta
        $numeroCuenta = self::sanitizarNumeroCuenta($form["empleado_numero_cuenta"]);
        //beneficios adicionales
        $form["empleado_beneficios"] = isset($form["empleado_beneficios"]) ? $form["empleado_beneficios"] : [];
        $beneficiosAdicionales = self::sanitizarBeneficiosAdicionales($form["empleado_beneficios"]);
        //sanitizar usuario
    ...
    public static function sanitizarTelefono($telefono)
    {
        ...
        if (strlen($telefono) < 11 || strlen($telefono) > 16) {
            throw new InvalidArgumentException("El teléfono debe tener entre 11 y 16 caracteres");
        }
    ...
    public static function sanitizarMotivoBonificaciones($motivoBonificacion)
    {
        ...
        $motivoBonificacion = ucfirst(trim($motivoBonificacion));
    ...
    private static function sanitizarMontoDeduccion($monto)
    {
        ...
        if ($monto < 0) {
            throw new InvalidArgumentException("El monto de la deducción no puede ser negativo");
        }

        // Validar que sea múltiplo de 0.25 (opcional)
        if (fmod($monto, 0.25) !== 0.0) {
            throw new InvalidArgumentException("El monto debe ser múltiplo de 0.25");
        }

        // Redondear a dos decimales por consistencia
    ...
    private static function sanitizarBeneficiosAdicionales($beneficios)
    {
        ...
        // Si el campo no existe o no es array, usar vacío
        if (!is_array($beneficios)) {
            $beneficios = [];
        }

        // Filtrar y sanitizar
        $beneficiosFiltrados = array_filter($beneficios, function ($item) use ($beneficiosValidos) {
            return in_array(strtolower(trim($item)), $beneficiosValidos);
        });

        // Eliminar duplicados y reindexar
        $beneficiosUnicos = array_values(array_unique($beneficiosFiltrados));

        // Si está vacío, retornar "ninguno"
        if (empty($beneficiosUnicos)) {
            return "ninguno";
        }

        // Convertir a string separado por ", "
        return implode(", ", $beneficiosUnicos);
    }
    ...
    public static function sanitizarIDEmpleado($id)
    {
        ...
        // Acepta USR_ seguido de 6 a 10 caracteres alfanuméricos en mayúscula
        if (!preg_match('/^USR_[A-Z0-9]{6,20}$/', $id)) {
            throw new InvalidArgumentException("El ID de empleado debe tener el formato USR_ seguido de caracteres alfanuméricos en mayúscula");
        }
    ...
    public static function sanitizarIDEmpresa($id)
    {
        ...
        if (!preg_match('/^EMP_[A-Za-z0-9]{6,20}$/', $id)) {
            throw new InvalidArgumentException("El ID de la empresa debe tener el formato EMP_ seguido de caracteres alfanuméricos en mayúscula");
        }
