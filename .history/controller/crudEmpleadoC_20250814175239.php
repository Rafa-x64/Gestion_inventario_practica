...
class crudEmpleadoC extends mainModel
{
    ...
    public static function obtener_formulario($form): array
    {
        ...
        //sanitizar telefono
        $form["empleado_telefono"] = trim($form["empleado_telefono"]);
    ...
    private function sanitizarTelefono($telefono)
    {
        if (!is_string($telefono)) {
            throw new InvalidArgumentException("el telefono debe ser una cadena de texto");
        }

        if(strlen($telefono) < 13 || strlen($telefono) > 16){
            throw new InvalidArgumentException("el telefono debe tener entre 13 y 16 caracteres");
        }
    }
