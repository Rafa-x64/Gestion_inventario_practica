            ...
            <div class="row">
                ...
                <div class="col-11 d-flex flex-column align-items-center justify-content-center">
                    ...
                    <form action="" method="post">
                        (input:text#nombre_legal[placeholder="Nombre legal"])+(select>option*6)+(input:text#rif[placeholder="RIF"])+(input:text#direccion[placeholder="Direccion"])+(input:text#nombre_legal[placeholder="Nombre legal"])+(input:mail#correo[placeholder="Correo electronico"])+(input:text#nombre_representante[placeholder="Nombre del representante"])+(input:text#fecha_registro[placeholder="Fecha de registro"])+(select>option*2)+(input:text#logo)+
                    </form>
                </div>
            </div>
        </div>
